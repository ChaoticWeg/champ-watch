...
// prepare a "query" (key-pair object containing query name and URL)
var prepareQuery = function (name, baseURL, values) {
	return {
		name : 'riot#' + name,
		url  : formatURL(baseURL, values)
	};
};
...
var get = exports.get = function (query, callback) {
	// display URL. don't show the protocol or the riot api key.
	var dURL = query.url.replace(/^.*\/\//, '').replace(api_key, 'weg');
	console.log('{ name:"%s", url:"%s" }', query.name, dURL);

	client.get(query.url, function (data, response) {
...
exports.getChampInfoByID = function (id, callback) {
	var query = prepareQuery('getChampInfoByID', urls['champ-info'], { id: id });
	get(query, callback);
};

// callback:  function (data, response) { ... }
exports.getFreeChamps = function (callback) {
	var query = prepareQuery('getFreeChamps', urls['free-champs'], {});
	get(query, callback);
};

// callback: function (data, response) { ... }
exports.getAllChamps = function (callback) {
	var query = prepareQuery('getAllChamps', urls['all-champs'], {});
	get(query, callback);
};

// callback: function (version) { ... }
exports.getDataDragonVersion = function (callback) {
	var query = prepareQuery('getDataDragonVersion', urls['dd-version'], {});
	get(query, function (data, response) {
